...
const useStyles = makeStyles({//bien useStyles nhan vao mot prop la 1 obj co ten la wrapper, dung wrapper de style cho Card component
...
export default function HighlightCard({ title, count, type }) {//cac tham so nhan vao : title(tuong ung voi so ca mac), count(tuong ung voi so ca nhiem), type
  const classes = useStyles({ type });//khai bao bien classes va truyen 1 prop la type vao makeStyles 
  return (
    <Card className={classes.wrapper}> {/*gan style la classes.wrapper vao attribute la className thi card se co style tuong ung voi wrapper*/}
      <CardContent>
        ...
        <Typography variant='body2' component='span' className={classes.count}>{/**truyen vao className la classes.count de css */}
          <CountUp end={count} separator=' ' duration={2} />{/**boc count vao Countup de khi load trang no co hieu ung chu so trong count tang len chu ko in ra luon count
